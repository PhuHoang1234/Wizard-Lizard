...
public class PlayerController3D : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        rb.velocity = new Vector3(newVelXZ.x, 0f, newVelXZ.y);

        if (desiredVelXZ.sqrMagnitude > 0.0001f && rb.velocity.sqrMagnitude > 0.0001f)
        {
            ...
            float lateralSpeed = Vector3.Dot(rb.velocity, right3);
            float reduce = Mathf.MoveTowards(lateralSpeed, 0f, lateralFriction * Time.fixedDeltaTime);
            rb.velocity += right3 * (reduce - lateralSpeed);
        }
